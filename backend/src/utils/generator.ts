...
class Generator {
    ...
    generateEvent = async (
        ...
        const dRange = dateRange ?? [0, 10];
        const cRange = costRange ?? [0, 30];

        const venueId = await this.generateVenue();
        const start_time = new Date();
        start_time.setDate(start_time.getDate() + randomIntRange(dRange));
        const cost = randomIntRange(cRange);
    ...
    generateUser = async (withSession?: boolean): Promise<string> => {
        if (NODE_ENV != "test")
